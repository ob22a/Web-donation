...
export async function emailDonationReceipt(req, res) {
  try {
    ...
    // Correctly call the service with the donationId
    const emailResult = await sendDonationReceipt(donationId);

    if (!emailResult.success) {
      return sendJson(res, 500, {
        message: emailResult.error || "Failed to send receipt",
      });
    }

    // Re-fetch updated donation to return the new status to the frontend
