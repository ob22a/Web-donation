...
const NGOs = () => {
  ...
  const prevNGO = () => {
    if (filteredNGOs.length === 0) return;
    setCurrentIndex(
      (prev) => (prev - 1 + filteredNGOs.length) % filteredNGOs.length,
    );
  };
  ...
  const openModal = async (ngo) => {
    ...
    if (
      ...
      setToastMessage(
        "Please add a payment method in your profile settings before donating.",
      );
      setShowToast(true);
    ...
    try {
      ...
      if (response.campaigns) {
        ...
        const activeOnes = response.campaigns.filter(
  ...
  const handleDonate = async (e) => {
    ...
    try {
      ...
      if (response.newDonation) {
        ...
        setToastMessage(
          `Success! You donated ${donationData.amount} ETB to ${selectedNGO.ngoName || selectedNGO.name}`,
        );
        setShowToast(true);
        closeModal();
      }
        ...
        filteredNGOs.length > 0 ? (
          ...
          >
            ...
            <article className="ngo-card" style={{ flex: 1 }}>
              ...
              <div className="ngo-card-body">
                <h2 className="ngo-card-title">
                  {currentNGO.ngoName || currentNGO.name}
                </h2>
                <p className="ngo-card-description">
                  {currentNGO.description || "No description provided."}
                </p>
                <div
